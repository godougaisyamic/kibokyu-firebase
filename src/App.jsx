...
const TYPES = ["全休", "午前休", "午後休", "その他"];
const DEFAULT_TYPE = "全休";
const STATUS_COLOR = {
...
function EmployeeView({ employees, requests, saveRequests, notify }) {
  ...
  const [newDateTypes, setNewDateTypes] = useState({});
  ...
  const toggleDate = (cell) => {
    ...
    setNewDateTypes((prev) => ({ ...prev, [cell.key]: DEFAULT_TYPE }));
  };

  const setTypeFor = (dateKey, type) => {
    setNewDateTypes((prev) => ({ ...prev, [dateKey]: type }));
  };
  ...
  const submit = async () => {
    ...
    const created = newDates.map((d) => ({
      ...
      name,
      date: d,
      type: newDateTypes[d] || DEFAULT_TYPE,
      status: "pending",
      ...
      {newDates.length > 0 && (
        <div className="rounded-2xl p-4" style={{ backgroundColor: THEME.surface, boxShadow: "0 1px 2px rgba(28,35,51,0.06)" }}>
          ...
          <div className="space-y-2.5">
            {newDates.map((d) => (
              <div key={d}>
                <div className="text-sm font-bold tabular-nums mb-1" style={{ color: THEME.ink, fontFamily: FONT_NUM }}>
                  {d.slice(5).replace("-", "/")}
                </div>
                <div className="grid grid-cols-4 gap-1.5">
                  {TYPES.map((t) => {
                    const active = (newDateTypes[d] || DEFAULT_TYPE) === t;
                    return (
                      <button
                        key={t}
                        onClick={() => setTypeFor(d, t)}
            ...
            {myExisting.map((r) => (
              <div key={r.id} className="flex items-center justify-between rounded-xl px-3 py-2" style={{ backgroundColor: THEME.surfaceAlt }}>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs" style={{ color: THEME.inkSub }}>
                    {r.type || DEFAULT_TYPE}
                  </span>
                </div>
                <div className="flex items-center gap-2">
...
function AdminView({ employees, saveEmployees, requests, saveRequests, adminPin, saveAdminPin, notify }) {
  ...
  const exportExcel = () => {
    ...
    ws["!cols"] = [{ wch: 14 }, { wch: 12 }, { wch: 10 }, { wch: 10 }, { wch: 20 }];
                ...
                <div key={r.id} className="px-4 py-3 flex items-center justify-between gap-2">
                  <div className="min-w-0">
                    <div className="flex items-center gap-1.5 min-w-0">
                      <span className="font-medium text-sm truncate" style={{ color: THEME.ink }}>
                        {r.name}
                      </span>
                      <span
